...
--        followers_count = (SELECT COUNT(*) FROM followers WHERE followed_user_id = NEW.user_id),  -- 假设有一个 followers 表
--        following_count = (SELECT COUNT(*) FROM followers WHERE follower_user_id = NEW.user_id),
        ...
        engagement_rate = (SELECT AVG(like_count::float / NULLIF(like_count + dislike_count, 0)) FROM posts WHERE user_id = NEW.user_id)

...
// Filtered at render time so signing in (e.g. from the dialog) drops yourself from the list.
const suggested = computed(() => people.value.filter(user => user.id !== auth.user?.id).slice(0, 4));
...
const setTab = async (value: Tab) => {
  if (value === "following" && !(await auth.ensure("following"))) return;
  router.push({ query: { ...route.query, tab: value === "all" ? undefined : value } });
};
...
const loadSidebar = async () => {
  ...
  if (users.status === "fulfilled") people.value = users.value.body?.data || [];
...
<template>
  <div class="ax-container grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_18rem]">
      ...
      <section v-else class="ax-panel">
        <div class="card-body flex-row flex-wrap items-center gap-3 p-4">
          <div class="min-w-0 flex-1">
            <h2 class="font-bold">{{ t("home.signInTitle") }}</h2>
            <p class="ax-muted text-sm">{{ t("home.signInPrompt") }}</p>
          </div>
          <div class="flex gap-2">
            <button class="btn btn-ghost btn-sm" @click="auth.ensure('post')">{{ t("nav.signIn") }}</button>
            <RouterLink :to="{ name: 'register', query: { redirect: route.fullPath } }" class="btn btn-primary btn-sm">{{ t("nav.register") }}</RouterLink>
          </div>
        </div>
      </section>
      ...
      <div class="sticky top-16 z-30 flex flex-wrap items-center justify-between gap-2 rounded-box border border-base-300 bg-base-100/95 px-3 py-2 backdrop-blur md:top-[4.25rem]">
        ...
        <div class="flex items-center gap-1">
          ...
          <div role="tablist" class="tabs tabs-box tabs-sm">
            ...
            <button role="tab" class="tab gap-1" :class="{ 'tab-active': tab === 'following' }" :title="auth.user ? '' : t('home.followingHint')" @click="setTab('following')"><UsersRound :size="14" /> {{ t("home.following") }}</button>
          </div>
    ...
    <aside class="min-w-0 space-y-4">
      ...
      <SectionCard v-if="suggested.length" :title="t('home.whoToFollow')" :icon="UsersRound" to="/people" :action="t('home.seeAll')">
        <UserRow v-for="person in suggested" :key="person.id" :user="person" />
      </SectionCard>
    </aside>

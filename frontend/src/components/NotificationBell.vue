...
<template>
  <div class="dropdown dropdown-end">
    <button tabindex="0" class="btn btn-ghost btn-circle" :aria-label="t('nav.notifications')" @click="load">
      <div class="indicator"><Bell :size="19" /><Transition name="pop"><span v-if="unread" class="badge badge-error badge-xs indicator-item">{{ unread > 99 ? "99+" : unread }}</span></Transition></div>
    </button>
    <div tabindex="0" class="dropdown-content card card-border z-50 mt-3 w-80 bg-base-100 shadow-2xl">
      <div class="card-body gap-2 p-3">
        <div class="flex items-center justify-between px-1"><h3 class="font-bold">{{ t("notifications.title") }}</h3><button class="btn btn-ghost btn-xs" :disabled="!unread" @click="readAll"><CheckCheck :size="15" /> {{ t("notifications.readAll") }}</button></div>
